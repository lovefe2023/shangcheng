...
export default function AdminEditGroupBuy() {
  const { id } = useParams();
  ...
  const handleSave = async () => {
    ...
    if (!formData.group_price || parseFloat(formData.group_price) <= 0) {
      toast.error('请输入正确的团购价');
      return;
    }
    if (!formData.min_quantity || parseInt(formData.min_quantity) < 2) {
      toast.error('成团人数至少为2人');
      return;
    }
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">编辑团购活动</h1>
      </div>
        ...
        <div className="p-6 border-b border-slate-200 dark:border-slate-700">
          ...
          <div className="space-y-6">
                ...
                <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">原价: ¥{selectedProduct.price?.toLocaleString()}</p>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                  成团人数 <span className="text-red-500">*</span>
                </label>
                <input
                  type="number"
                  min="2"
